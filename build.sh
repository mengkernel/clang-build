...
BUILD_START=$(date +"%s")
TOTAL_START=$(date +"%s")
./build-llvm.py --build-stage1-only --install-stage1-only --clang-vendor "$LLVM_NAME" --branch main --defines "$CUSTOM_FLAGS" --projects "clang;lld" --targets "AArch64;X86" --shallow-clone --build-type "Release" | tee build.log
BUILD_END=$(date +"%s")
