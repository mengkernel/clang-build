...
BUILD_START=$(date +"%s")
./build-llvm.py --build-stage1-only --install-stage1-only --clang-vendor "$LLVM_NAME" --branch "$BRANCH" --defines "$CUSTOM_FLAGS" --projects "clang;lld;polly;openmp" --targets "ARM;AArch64;X86" | tee build.log
BUILD_END=$(date +"%s")
